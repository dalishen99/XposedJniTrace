...
string checkSign(JNIEnv * env,jobject context){
    jmethodID getApplicationInfo_id = env->GetMethodID(env->GetObjectClass(context),
                                                       "getApplicationInfo",
                                                       "()Landroid/content/pm/ApplicationInfo;");
    jobject info = env->CallObjectMethod(context, getApplicationInfo_id);
    jfieldID pId = env->GetFieldID(env->GetObjectClass(info),
                                   "publicSourceDir", "Ljava/lang/String;");
    const char *capkPath =
            env->GetStringUTFChars((jstring)env->GetObjectField(info,pId), NULL);
    int fd1 = my_openat(AT_FDCWD,
                     reinterpret_cast<const char *>(capkPath),
                     O_RDONLY | O_CLOEXEC,
                     0640
                     );
    if(fd1==-1){
    ...
    std::string md5Result = Md5Utils(read_certificate(fd1)).hexdigest();
    close(fd1);
    return md5Result;
}
